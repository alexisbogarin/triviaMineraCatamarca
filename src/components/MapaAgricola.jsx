        ...
        <div className="h-full">
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden h-[calc(100%-80px)]">
                <ComposableMap
                    ...
                    projectionConfig={{ scale: 5000, center: [-59.5, -32] }}
                    ...
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                        <div className="p-6">
                            ...
                            <div className="text-center mb-6">
                                ...
                                <h3 className="text-xl text-blue-600 mt-1">
                                    {departamento.departamento}
                                </h3>
                                <p className="text-gray-500">{departamento.cabecera}</p>
                            </div>
                            ...
                            <div className="flex border-b border-gray-200 mb-4">
                                <button
                                    ...
                                    className={`px-4 py-2 font-medium ${modoVisualizacion === "siglea" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500"}`}
                                >
                                    Producción
                                </button>
                                <button
                                    onClick={() => onModoChange("senasa")}
                                    className={`px-4 py-2 font-medium ${modoVisualizacion === "senasa" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500"}`}
                                >
                                    Rebaño
                                </button>
                                <button
                                    onClick={() => onModoChange("industrias")}
                                    className={`px-4 py-2 font-medium ${modoVisualizacion === "industrias" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500"}`}
                                >
                                    Industrias
                                </button>
                            </div>
                                                ...
                                                <div key={i} className="space-y-4">
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <div className="bg-blue-50 p-3 rounded-lg">
                                                            <p className="text-sm text-blue-600">Litros de leche</p>
                                                            <p className="text-xl font-bold">{data.litros?.toLocaleString() || 'N/D'}</p>
                                                            <p className="text-xs text-gray-500">{data.porcentaje_litros ? (data.porcentaje_litros * 100).toFixed(1) + '% provincial' : ''}</p>
                                                        </div>
                                                        <div className="bg-green-50 p-3 rounded-lg">
                                                            <p className="text-sm text-green-600">Litros por tambo</p>
                                                            <p className="text-xl font-bold">{data.litros_por_tambo?.toLocaleString('es-AR', { maximumFractionDigits: 0 }) || 'N/D'}</p>
                                                        </div>
                                                    </div>

                                                    <div className="bg-white border rounded-lg p-4">
                                                        <h4 className="font-medium mb-2">Composición</h4>
                                                        <div className="grid grid-cols-2 gap-4">
                                                            <div>
                                                                ...
                                                                <p className="text-sm font-medium mt-1">{data.porcentaje_grasa?.toFixed(2) || 'N/D'}%</p>
                                                            </div>
                                                            <div>
                                                                ...
                                                                <p className="text-sm font-medium mt-1">{data.porcentaje_proteina?.toFixed(2) || 'N/D'}%</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div key={i} className="space-y-4">
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <div className="bg-purple-50 p-3 rounded-lg">
                                                            <p className="text-sm text-purple-600">Vacas en ordeñe</p>
                                                            <p className="text-xl font-bold">{data.vacas?.toLocaleString() || 'N/D'}</p>
                                                            <p className="text-xs text-gray-500">{data.porcentaje_vacas ? (data.porcentaje_vacas * 100).toFixed(1) + '% provincial' : ''}</p>
                                                        </div>
                                                        <div className="bg-indigo-50 p-3 rounded-lg">
                                                            <p className="text-sm text-indigo-600">Total bovinos</p>
                                                            <p className="text-xl font-bold">{data.total_bovinos?.toLocaleString() || 'N/D'}</p>
                                                        </div>
                                                    </div>

                                                    <div className="bg-white border rounded-lg p-4">
                                                        <h4 className="font-medium mb-2">Establecimientos</h4>
                                                        <div className="space-y-3">
                                                            <div>
                                                                <p className="text-sm text-gray-500">N° RENSPA</p>
                                                                <p className="text-lg font-medium">{data.numero_renspa || 'N/D'}</p>
                                                            </div>
                                                            <div>
                                                                <p className="text-sm text-gray-500">Vacas por establecimiento</p>
                                                                <p className="text-lg font-medium">{data.vacas_por_tambo?.toFixed(1) || 'N/D'}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            .map((data, i) => (
                                                <div key={i} className="space-y-4">
                                                    <div className="bg-cyan-50 p-3 rounded-lg">
                                                        <p className="text-sm text-cyan-600">Industrias lácteas</p>
                                                        <p className="text-3xl font-bold">{data.numero_industrias || 'N/D'}</p>
                                                        <p className="text-xs text-gray-500">{data.porcentaje_industrias ? (data.porcentaje_industrias * 100).toFixed(1) + '% provincial' : ''}</p>
                                                    </div>

                                                    <div className="bg-white border rounded-lg p-4">
                                                        <h4 className="font-medium mb-2">Distribución provincial</h4>
